...
Bun tests live next to the modules they cover, for example `src/core/workspace.test.ts` and `src/main.test.ts`. When adding logic beyond simple UI wiring, add focused tests and run `bun test`. Before opening a PR, run `bun run build` and `bun run validate`; use `bun run verify` for the combined test, build, and manifest check.
